...
mice.impute.ml.lmer <- function(y, ry, x, type, levels_id , variables_levels=NULL,
				random_slopes = NULL , aggregate_automatically = TRUE , intercept = TRUE , groupcenter.slope=FALSE, 
				draw.fixed=TRUE, random.effects.shrinkage=1E-6, glmer.warnings=TRUE, 
				model = "continuous" , donors = 3 , match_sampled_pars = FALSE , blme_use = FALSE ,
				blme_args = NULL , pls.facs=0, interactions = NULL , quadratics = NULL ,
				min.int.cor=0, min.all.cor=0,
				pls.print.progress = FALSE ,  ... )
{
	...
	res <- mice.impute.2l.pls2( y=y, ry=ry, x=x, type=type, pls.facs=pls.facs, 
				pls.print.progress=pls.print.progress, min.int.cor=min.int.cor, envir_pos=pos, 
				min.all.cor=min.all.cor, pls.impMethod='xplsfacs' ) 
	x <- res[,-1,drop=FALSE]
	if (pls.facs > 0){
		nx <- ncol(x)
